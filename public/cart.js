...
function sizeCode(label) {
  // Map known sizes to short codes for the operator view.
  // 30×40 → M, 40×60 → L. Tolerant of spaces, x/×/*, and cm/سم suffixes.
...
async function handleCartOrder(req, res, supabase, body) {
  ...
  // Cart line items now render in the operator's Order section (from the `items`
  // column), so the notes field only carries the customer's own note (if any).
